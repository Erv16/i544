...
class Sensors {
  ...
  async addSensor(info) {
    ...
    if (sensor.model && !this.sensorTypes[sensor.model]) {
      throw [ `unknown model "${sensor.model}"` ];
    }
  ...
  async addSensorData(info) {
    ...
    if (sensorData.sensorId && !this.sensors[sensorData.sensorId]) {
      throw `unknown sensor id "${sensorData.sensorId}"`;
    }
    let data = this.sensorData[sensorData.sensorId];
    const timestamp = Number(sensorData.timestamp);
    if (!data) {
      data = {
	...
	data: {},
      ...
      this.sensorData[sensorData.sensorId] = data;
    }
    if (timestamp < data.earliest) data.earliest = timestamp;
    if (timestamp > data.latest) data.latest = timestamp;
    data.data[sensorData.timestamp] = sensorData.value;
  }
  ...
  async findSensorTypes(info) {
    ...
    const { nextIndex, data } =
      findInCollection(this.sensorTypes, searchSpecs, 'sensor-type');
  ...
  async findSensors(info) {
    ...
    const { nextIndex, data } =
      findInCollection(this.sensors, searchSpecs, 'sensor');
  ...
  async findSensorData(info) {
    ...
    if (!sensor) throw [ `unknown sensor id "${sensorId}"` ];
    ...
    const data = [];
    for (let t = startTime;
	 t >= sensorData.earliest && data.length < count;
	 t = t - period) {
      const v = sensorData.data[String(t)];
      if (v === undefined) continue;
      const status =
	  !inRange(v, sensorType.limits) ? 'error'
	: !inRange(v, sensor.expected) ? 'outOfRange' : 'ok';
...
function findInCollection(collection, searchSpecs, name) {
  ...
  if (searchSpecs.id) {
    ...
    ids.sort();
    const {index, count} = searchSpecs;
    let i = (index < 0) ? 0 : index;
    nextData:
    for (i = i; i < ids.length && data.length < count; i++) {
