...
class Sensors {
  ...
  async addSensor(info) {
    ...
    if (sensorType === undefined) {
      throw [ new AppError('X_ID', `unknown sensor type "${model}"`, 'model') ];
    }
    const { min, max } = sensor.expected;
    if (!inRange(min, sensorType.limits)) {
      const { min: limMin, max: limMax } = sensorType.limits;
      const msg =
	`min value ${min} outside sensor-type limits [${limMin}, ${limMax}].`;
      throw new AppError('RANGE', msg, 'expected.min');
    }
    if (!inRange(max, sensorType.limits)) {
      const { min: limMin, max: limMax } = sensorType.limits;
      const msg =
	`max value ${max} outside sensor-type limits [${limMin}, ${limMax}].`;
      throw new AppError('RANGE', msg, 'expected.max');
    }
  ...
  async addSensorData(info) {
    ...
    if (sensor === undefined) {
      const err =
        new AppError('X_ID', `unknown sensor "${sensorId}"`, 'sensorId');
      throw [ err ];
    }
	...
	const msg = 'timestamp does not match sensor period';
	throw [ new AppError('BAD_TIMESTAMP', msg, 'timestamp') ];
      }
    }
  ...
  async findSensorTypes(info) {
    ...
    if (info.id !== undefined && data.length === 0) {
      const msg = `no results for sensor-type id '${info.id}'`;
      throw [ new AppError('NOT_FOUND', msg, 'id') ];
    }
    return { data,
  ...
  async findSensors(info) {
	...
	const sensorTypes = await this._get(this.sensorTypes,
					    { id: sensor.model, });
  ...
  async findSensorData(info) {
    ...
    const sensor = (await this._get(this.sensors, { id: sensorId, }))[0];
    if (!sensor) {
      const msg = `no sensor-type for id '${sensorId}'`;
      throw [ new AppError('NOT_FOUND', msg, 'sensorId') ];
    }
    const sensorType =
    ...
    if (!sensorTimestamps) {
      const msg = `no sensor data for sensor '${sensorId}'`;
      throw [ new AppError('NOT_FOUND', msg, 'sensorId') ];
    }
  ...
  async _get(collection, searchSpecs) {
    ...
    // else {
    {
      const count =  mongoInfo._count || 1;
      const index = mongoInfo._index || 0;
